...
// ModelToShortDTO converts a persistence model.User to a UserShort DTO
// Used by other entities that need User information in short format
func (m *UserMapper) ModelToShortDTO(user *model.User) *dto.UserShort {
	if user == nil {
		return nil
	}
	return &dto.UserShort{
...
// ModelToDTO converts a persistence model.User to UserResponse DTO
// Used for legacy compatibility with handlers that still use model directly
func (m *UserMapper) ModelToDTO(user *model.User) *dto.UserResponse {
	if user == nil {
		return nil
	}
...
// UpdateModelFromDTO updates a model.User with values from UpdateUserRequest DTO
// Used for legacy compatibility where direct model updates are needed
func (m *UserMapper) UpdateModelFromDTO(user *model.User, dto *dto.UpdateUserRequest) {
	if user == nil || dto == nil {
		return
	}
